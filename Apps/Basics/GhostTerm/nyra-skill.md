# Nyra Language — AI Assistant Reference (v1.2.x)
...
> **Project status — MVP, not production-ready:** Nyra is in **active development**. **Core** tier (types, control flow, tag enums, ownership, FFI) is semver-stable for learning and early evaluation. **Extended** tier, enum payloads, async, and **`?` error propagation are MVP or not shipped** — expect breaking changes and missing ergonomics. **Do not deploy Nyra for mission-critical or production workloads** until the roadmap clears production gates. See [Stability](#stability-v10) · `webDocs/roadmap.html`.
...
10. [Performance — release, PGO, escape analysis](#performance--optimization)
...
- **Default style:** write code **without** type annotations. The compiler infers locals, parameters, return types, struct shapes, and many generic call sites.
- **When to add types:** public APIs, FFI (`extern`), disambiguation when inference fails, or team preference. Annotations are **never required** unless the compiler cannot infer.
- **Zero runtime cost:** types exist only at compile time; inference does not slow down the binary.
- **Two valid styles, same language:** every learn-track example and most `examples/` files ship as **`foo.ny`** (easy, no annotations) and **`foo.typed.ny`** (explicit types). Docs default to **Without types** tabs; switch to **With types** when you want them.

```ny
// Easy (preferred for learning and scripts)
let score = 10
fn add(a, b) {
    return a + b
}

// Typed (optional — same program)
let score: i32 = 10
...
- **Extended (experimental):** **enum payloads** (`Some(i32)`, `Ok(T)` / `Err(E)`), `async`/`await`, traits, macros, **`defer`** (see [defer vs Drop](#defer-vs-drop--when-to-use-which)), explicit lifetimes, generics beyond monomorph MVP, `spawn { }`.
...
Release flags: `--release`, `--opt 0-3`, `--lto`, `--lto-full`, `--no-lto`, `--no-llvm-opt`, `--pgo-generate`, `--pgo-use FILE`, `--native-cpu` (host only), `--for`, `--os`, `--arch`, `--target`.
...
| f64 | Copy | IEEE-754 double; literals like `3.14`, `1e-3` |
...
Backtick strings with `{expr}` interpolation (static text + `i32` / `string` values):
...
**Result / error handling — documented MVP (no `?` operator):**

| Status | Detail |
|--------|--------|
| **Shipped (MVP)** | Monomorph `Result_i32_i32` / `import "stdlib/option.ny"` payloads; explicit `match` on `Ok` / `Err`; `unwrap_*` helpers in `stdlib/result.ny` |
| **Not shipped** | `?` operator, early `return` inside `match` arms, ergonomic `try` blocks |
| **Production use** | **Not recommended** — verbose error paths and incomplete codegen for some `Result` return patterns; use for learning and conformance-tested demos only |

With a real `Result` payload, **every fallible step needs its own `match`**. Nyra does not ship Rust-style `?` propagation yet, so pipelines become repetitive quickly — acceptable for MVP samples, painful for large apps until `?` lands.
...
// Three steps → three matches (or deeply nested match). No `return` inside match arms yet.
...
Nested form (same semantics, harder to read):
...
| Approach | When |
|----------|------|
| Explicit `match` per step | Default — required today |
| `unwrap_*` helpers | `stdlib/result.ny` — e.g. `unwrap_i32_result(r, default)` |
| Custom `bind_*` wrapper | Hide one match inside a helper function |
| `?` operator | **Not implemented** — on the roadmap; early-return inside `match` arms is also not supported yet |

**Why it matters:** Application code with many fallible calls (parse → validate → open → read) will accumulate `match` boilerplate until `?` or equivalent lands. Plan for helper functions or keep pipelines shallow.

Tests: `tests/nyra/result_propagate_test.ny` · Example: `examples/result_propagate_verbose.ny` · Conformance: `CONF-ADT-003`

Generic `Result<T,E>` via `import "stdlib/option.ny"` follows the same rule — match per step, no `?`.
...
- Import brings functions, structs, enums, consts into scope.

## I/O & builtins

See full runnable gallery: `examples/builtins/` · `webDocs/stdlib.html#builtins`

### I/O (no import)

```ny
print("line")                    // stdout + newline (string, i32, bool, char, f64)
...
| `.replace(from, to)` | 2 × `string` | `string` | First match only |
...
|--------|---------|-------|
| **Shipped** | `vec.ny`, `map.ny`, `collections/*`, `strings/ops.ny`, `fs/mod.ny`, `path.ny`, `net/tcp.ny`, `net/http/mod.ny`, `tls.ny`, `time/*`, `random_bytes` | Collections, FS, HTTP/TCP, builtins |
| **MVP / partial** | `json/mod.ny` (single-field today), `uuid/mod.ny`, `url` helpers, `async.ny`, `reflect/mod.ny`, `bridge/mod.ny` | Expanding toward full serde |
| **Stub → in progress** | `crypto/mod.ny`, `db/sqlite.ny`, `net/websocket.ny`, `compress/mod.ny`, `serialize/mod.ny`, `encoding/mod.ny`, `env_set`, `stdin_read`, `Mutex_i32`, `UdpSocket`, `Command.run` | Native `stdlib/rt/` implementations landing without import-path changes |
...
| String-key map | `HashMap_str_i32_*`, `Map_str_i32_*` in `map.ny` | `HashMap<K,V>` (Extended) | **Method chaining works:** `HashMap_str_i32_new().insert("a", 1).insert("b", 2)` · or low-level `ptr` + `nyra_map_*` externs |
...
**Stdlib auto-prelude:** All `stdlib/**/*.ny` symbols are merged automatically — use `read_file`, `Vec_i32_new`, `StrVec`, etc. without imports. Opt out with `# no_std`. Explicit `import "stdlib/vec.ny"` still works.

**Core modules (usable):** `vec.ny`, `map.ny`, `collections/*`, `strings/ops.ny`, `strings/regex.ny`, `fs/mod.ny`, `path.ny`, `time/instant.ny`, `time/date.ny`, `json/mod.ny` (MVP), `iter/mod.ny`, `env/mod.ny` (`env_get`/`env_has`), `config/mod.ny`, **`net/http/mod.ny`**, `net/tcp.ny`, `tls.ny`, `bridge/mod.ny`, `bench/mod.ny`, `profile/mod.ny`, `testing.ny`, `async.ny` (Extended). Legacy: `http/mod.ny` re-exports `net/http`.

**In progress (stubs landing native code):** `crypto/mod.ny`, `db/sqlite.ny`, `net/websocket.ny`, `compress/mod.ny`, `serialize/mod.ny` (full serde), `encoding/mod.ny` (`base64_*`), `env_set`, `terminal/mod.ny` (`stdin_read`), `process.ny`, `Mutex_i32`, `UdpSocket`. See `stdlib/README.md`.
...
**Low-level runtime** (still valid): `read_file`, `nyra_vec_i32_*`, `nyra_map_str_i32_*`, `nyra_channel_*`, `bridge_exec`, `spawn { }`.
...
| Bundled dev copy | `examples/packages/ny-sqlite` when in Nyra repo |
...
| Layer | Role | Example |
|-------|------|---------|
| **Nyra runtime** | Bootstrap I/O, strings, spawn, channels | `stdlib/rt/*.c` → `nyra_*` ABI |
...
`import "stdlib/core/mem.ny"` for `malloc`, `free`, `memcpy`, `memset`, `nyra_volatile_*`.
...
- **`Drop` / `Clone`** traits use dedicated compiler paths; user traits use the generic vtable MVP.

### Limitations (MVP)

- Copy-sized structs only (heap box via `malloc` + `memcpy`).
- No trait bounds on generics, no `dyn Trait + Send`, no multi-trait objects.
...
Example: `examples/trait_dyn.ny` · tests: `tests/nyra/trait_dispatch_test.ny`
...
- No **`?` operator** (Result propagate) — only `??` nullish coalesce and `?.` optional chain.
...
- Async/`await` MVP: promise handles in `stdlib/rt/rt_async.c`; full state-machine scheduler not shipped. See `backend.html`.
- **Trait objects (`dyn Trait`)** — MVP shipped (vtable + box); no trait bounds / multi-trait objects yet. Static `impl Trait for Type` also Extended.
...
| Runtime bindings (`nyra_*` ↔ stdlib) | bindings.html |
