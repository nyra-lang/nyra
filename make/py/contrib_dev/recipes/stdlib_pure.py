...
def apply(spec: StdlibFnSpec, *, force: bool = False) -> RecipeResult:
    ...
    ex_rel = ex_path.relative_to(EXAMPLES.parent) if ex_path.is_relative_to(EXAMPLES.parent) else ex_path
...
def _test_spec(spec: StdlibFnSpec, name: str):
    ...
    return TestExampleSpec(
        ...
        use_testing=True,
        example_topic=builtin_example_topic(spec.ny_module),
    )
