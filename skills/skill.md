...
(Varies by machine. Time auto-picks `s` / `ms` / `µs` / `ns` so tiny blocks are not rounded to `0.0 ms`. **Memory** is process RSS delta — a stack `i32` will usually stay `0 B`; use `size_of_i32()` / `size_of<T>()` for type layout size.)
