...
impl Codegen {
    ...
    pub(super) fn compile_parallel_search(
        &mut self,
        ps: &ParallelSearchExpr,
        env: &Env,
    ) -> super::ExprValue {
        let par_idx = self.func_par_idx;
        self.func_par_idx += 1;
        let safe_func = self.current_func.replace('.', "_");
        let body_symbol = format!("__par_pred_{safe_func}_{par_idx}");
        let cap_ty_name = format!("ParCtx.{safe_func}.{par_idx}");

        let outer: HashSet<String> = env.keys().cloned().collect();
        let mut declared = outer.clone();
        declared.insert(ps.var.clone());
        let capture_names: Vec<String> = ownership::collect_captures(&ps.body, &declared)
            .into_iter()
            .filter(|n| *n != ps.var)
            .collect();
        ...
        let (max_w, exact_w, mode, cpu_pct, backend) =
            self.compile_parallel_opts(&ps.config, env);

        let runtime_sym = match ps.config.op {
            ParallelOp::Any => "parallel_any_range",
            ParallelOp::Find => "parallel_find_range",
            ParallelOp::All => "parallel_all_range",
            ParallelOp::Iterate => "parallel_any_range",
        };
        ...
        self.emit_parallel_pred_fn(
            &body_symbol,
            &format!("{}__par_{par_idx}", self.current_func),
            &ps.body,
            &cap_ty_name,
            &fields,
            &plan,
        ...
        let result_reg = if llvm_fields.is_empty() {
            let r = self.fresh("par.search");
            self.emit_runtime_call(
                runtime_sym,
                &format!(
                    "  %{r} = call i32 @{runtime_sym}(i32 {start_op}, i32 {end_op}, ptr @{body_symbol}, ptr null, i32 {max_w}, i32 {exact_w}, i32 {mode}, i32 {cpu_pct}, i32 {backend})"
                ),
            );
            r
        } else {
            ...
            self.emit_runtime_call(
                runtime_sym,
                &format!(
                    "  %{r} = call i32 @{runtime_sym}(i32 {start_op}, i32 {end_op}, ptr @{body_symbol}, ptr %{ctx_ptr}, i32 {max_w}, i32 {exact_w}, i32 {mode}, i32 {cpu_pct}, i32 {backend})"
                ),
            );
            r
        };

        if ps.config.op == ParallelOp::Find {
            return super::ExprValue {
                reg: format!("%{result_reg}"),
                ty: "i32".into(),
            };
        }
        let bool_reg = self.fresh("par.search.bool");
        self.emit(&format!(
            "  %{bool_reg} = icmp ne i32 %{result_reg}, 0"
        ));
        super::ExprValue {
            reg: format!("%{bool_reg}"),
            ty: "i1".into(),
    ...
    fn emit_parallel_pred_fn(
        ...
        self.emit_buf = Some(Vec::new());
        let nested_scope = self.push_nested_fn_codegen_scope();
        self.emit(&format!("define i32 @{symbol}(i32 %idx, ptr %raw) {{"));
        self.emit("entry:");
        let mut env: Env = HashMap::new();
        let cap_ptr = if !captures.is_empty() {
            let cap_ptr = self.fresh("par.cap.bc");
        ...
        let mut par_drop = DropState::new(drop_func);
        let val = self.compile_block_as_expr(body, &env, &mut par_drop);
        let mut op = if val.reg.starts_with('%') {
            val.reg.clone()
        } else {
            format!("%{}", val.reg)
        };
        if val.ty == "i1" {
            let ext = self.fresh("par.pred.zext");
            self.emit(&format!("  %{ext} = zext i1 {op} to i32"));
            op = format!("%{ext}");
        }
        self.emit(&format!("  ret i32 {op}"));
        self.emit("}");
        self.pop_nested_fn_codegen_scope(nested_scope);
        if let Some(helper) = self.emit_buf.take() {
            self.module_level.extend(helper);
